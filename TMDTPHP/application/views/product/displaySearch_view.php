...
<body id="top" class="display-search" style="background: black;color:white !important">
	...
	<div class="container " >
		<h1 class="alert alert-success text-uppercase mt-3 text-center" style="font-size: 2em">Kết quả tìm kiếm</h1>
		<h5 class="text-left">Từ khóa: <?= $_GET['q'] ?></h5>
		<h6>---</h6>

		<?php if(empty($products)) { ?>
			
			<h4 class="alert alert-danger text-center">Không có sản phẩm nào khớp với tìm kiếm này</h4>
			...
			<ul class="pagination">
				<?php if($numpage != 0): ?>
					...
					<li class="page-item <?= $currentpage == 1 ? 'disabled' : '' ?>">
						<a class="page-link" href="<?= $currentpage > 1 ? base_url().'tim-kiem.html?q='.strtolower($_GET['q']).'&p='.($currentpage - 1) : '#' ?>" <?= $currentpage == 1 ? 'tabindex="-1"' : '' ?> >Trước</a>
					</li>

				<?php endif; ?>
				<?php for($i = 1 ; $i <= $numpage ; $i++ ): ?>

				<li class="page-item <?= $currentpage == $i ? 'active' : '' ?>"><a class="page-link" <?= $currentpage != $i ? 'href="'.base_url().'tim-kiem.html?q='.strtolower($_GET['q']).'&p='.$i.'"' : '' ?> "><?= $i ?></a></li>
				...
				<li class="page-item">
					<a class="page-link" href="<?= base_url().'tim-kiem.html?q='.strtolower($_GET['q']).'&p='.($currentpage + 1) ?>">Sau</a>
				</li>

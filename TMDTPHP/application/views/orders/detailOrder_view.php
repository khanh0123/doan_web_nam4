...
<head>
    ...
    <title>Giày store- Chi tiết đơn hàng</title>
    <meta charset="utf-8">
    ...
    <!-- End Font awesome -->
    <style>body,html{width: auto;height: auto;background-color:#eff0f4;}.info-address,.info-total{background: white;color:  black;padding:  1em;}</style>
...
<body id="top">
    ...
    <div id="detail-order">
       <div class="container">
            <h2 class="alert alert-success text-center mt-3">Chi tiết đơn hàng</h2>
            <?php foreach ($dataorder as $value): ?>
            <?php $cart = $value['detail']['cart'];  ?>
            <?php $info_payment = $value['detail']['info_payment'];  ?>

                
            
            <div class="info-order mb-2">
                <div class="row info-main">
                    <div class="col-6 order-code text-left" >Đơn hàng <a href="<?= base_url() ?>chitietdonhang/<?= $value['id'] ?>"><span class="code">#<?= $value['id'] ?></span></a><br>Đặt ngày <span>18/4/2018</span>
                    </div>
                    <div class="col-6 view-detail text-right">
                        <?php if( $value['status'] != "Đã hủy" && $value['status'] != "Đã giao hàng"){ ?>

                            <a class="<?= $value['status'] != "Chờ xác nhận" && $value['status'] != "Chờ thanh toán" ? 'disabled' : '' ?>"  href="<?= $value['status'] != "Chờ xác nhận" && $value['status'] != "Chờ thanh toán" ? '#'  : base_url().'huydonhang/'.$value['id'] ?>">Hủy đơn hàng</a>

                        <?php }else if ($value['status'] == "Đã giao hàng"){?>
                ...
                <div class="table-responsive">
                    <table class="table detail-info" style="width: 100%">
                        ...
                        <tr>
                            ...
                            <td>
                                <span><?= number_format(isset($product['price']) ? $product['price'] : 0) ?> <sup>đ</sup></span>
                            </td>
                            ...
                            <?php if(!empty($redirectto)): ?>
                               <td class="title">
                                    <a target="_blank" href="<?= $redirectto ?>" class="hidden-xs-up btn btn-success btn-thanh-toan" >Thanh toán ngay</a>
                                </td>
                ...
                <div class="row" >
                    <div class="col-md-6 pr-md-1" >
                        <div class="info-address" >
                            <h5 class="text-left">Địa chỉ giao hàng</h5>
                            <div class="person-name"><?= $info_payment['name'] ?></div>
                            <div class="person-address"><?= $info_payment['address'] ?></div>
                            <div class="person-address"><?= $info_payment['phone'] ?></div>
                        </div>
                    </div>
                    <div class="col-md-6 pl-md-1">
                        <div class="info-total"  >
                            ...
                            <div class="temp-money">
                                <span >Tạm tính</span>
                                <span class="float-right"><?= number_format($value['detail']['total_money']) ?> <sup>đ</sup></span>
                            </div>

                            <div class="shipping-money clearfix">
                                <span class="float-left">Phí vận chuyển</span>
                                <span class="float-right"><?= number_format($value['detail']['shipping']) ?> <sup>đ</sup></span>
                            </div>
                            <div class="line mt-2 mb-2" style="border: 1px solid gray"></div>
                            <div class="shipping-money clearfix">
                                <span class="float-left">Tổng cộng(Bao gồm VAT)</span>
                                <span class="float-right"><?= number_format($value['detail']['total_money']+$value['detail']['shipping']) ?> <sup>đ</sup></span>
                            </div>
                            <div class="method-payment">
                                Thanh toán bởi <span><?= $value['payment'] ?></span>
                            </div>
                        </div>
